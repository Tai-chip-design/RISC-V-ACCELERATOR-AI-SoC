...
Requires ModelSim/Questa (`vsim`, `vlog`).

```bash
vlib work
vlog -sv rtl/cpu/*.v rtl/memory/*.v rtl/accelerator/*.v rtl/soc/*.v tb/soc/tb_soc_top.v
vsim -c work.tb_soc_top -do "run -all; quit"
```

Each module under `tb/` can be run independently against its corresponding file in `rtl/` for isolated verification.
